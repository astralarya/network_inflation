...
parser.add_argument('imagenet_repo', type=Path, help="Huggingface imagenet repo path generated by `git clone https://huggingface.co/datasets/imagenet-1k`")
...
shutil.copy(args.imagenet_repo.joinpath("classes.py"), args.imagenet_path.joinpath("classes.py"))
...
for filename in get_files("test"):
    ...
    with tarfile.open(filename, "r:gz") as tar:
        for member in tqdm(tar.getmembers()):
            with tar.extractfile(member) as extract:
                with args.imagenet_path.joinpath("test", name).open(mode="wb") as output:
                    output.write(extract.read())
